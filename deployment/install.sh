...
function usage()
{
    echo "'Gamestation-manager' installer"
    echo ""
    echo "./install.sh"
    echo "    -h --help"
    echo ""
    ...
    echo ""
    echo "    --compatible"
    echo "      Optionnal, use requirements compatible with Gamestation 3.2.11"
    echo ""
}
...
wget https://github.com/pmoran13800/Gamestation-manager/archive/$RELEASE.zip
unzip $RELEASE.zip
echo

echo "* Initialize virtual environment for project"
cd gamestation-manager-$RELEASE/
...
echo
echo "DONE!"
echo "Go into directory 'gamestation-manager-$RELEASE/' and you can start the server with the following command:"
echo
