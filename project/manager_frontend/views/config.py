...
class GamestationConfigFormView(FormView):
    """
    Display a form to edit the Gamestation config file
    """
    template_name = "manager_frontend/config_form.html"
    form_class = ConfigEditForm

    def dispatch(self, request, *args, **kwargs):
        self.config_filepath = settings.GAMESTATION_CONF_PATH
        self.config_content = self.get_config_file(self.config_filepath)
        
        self.backup_filepath = settings.GAMESTATION_CONF_BACKUP_PATH
        ...
        return super(GamestationConfigFormView, self).dispatch(request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        context = super(GamestationConfigFormView, self).get_context_data(**kwargs)
        context.update({
    ...
    def get_form_kwargs(self):
        context = super(GamestationConfigFormView, self).get_form_kwargs()
        context.update({
            'config_filepath': self.config_filepath,
            'backup_filepath': self.backup_filepath,
        })
    ...
    def form_valid(self, form):
        ...
        return super(GamestationConfigFormView, self).form_valid(form)
