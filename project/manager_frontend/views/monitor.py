...
GAMESTATION_THERMAL_BASEDIR = '/sys/class/thermal'
# Assume 'thermal_zone0' is the CPU thermal infos on Raspberry
GAMESTATION_THERMAL_DEVICE_CPU_DIR = 'thermal_zone0'
GAMESTATION_THERMAL_CURRENT_TEMP_FILE = 'temp'
GAMESTATION_THERMAL_MAXTEMP_LIMIT_FILE = 'trip_point_0_temp'


# Compatibility support for Gamestation versions from 3.2.x to 3.3.x
# (psutil package only available since 3.3.0 beta 5)
try:
    import psutil
except ImportError:
    class GamestationSystemInfosMixin(object):
    ...
    class GamestationSystemInfosMixin(object):
        ...
        mining_cpu_interval = settings.GAMESTATION_PSUTIL_CPU_INTERVAL
        ...
        def get_thermal_infos(self, device_dir):
            ...
            """
            critical_limit = float(open(os.path.join(device_dir, GAMESTATION_THERMAL_MAXTEMP_LIMIT_FILE), 'r').read().strip())
            current_temp = float(open(os.path.join(device_dir, GAMESTATION_THERMAL_CURRENT_TEMP_FILE), 'r').read().strip())
...
class MonitoringView(GamestationSystemInfosMixin, TemplateView):
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.psutil_available:
            context.update({
                ...
                'cpu_thermal_infos': self.get_thermal_infos(os.path.join(GAMESTATION_THERMAL_BASEDIR, GAMESTATION_THERMAL_DEVICE_CPU_DIR)),
            })
        return context
