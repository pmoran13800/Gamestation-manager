...
* Moved 'Gamestation manifest' stuff into ``project.gamestation_manifest``, removed manifest loading from settings in profit of loading from ``urls.py``;

Version 1.0.2 - 2015/10/16
--------------------------

* Fixed CRLF caracters when editing gamestation config, close #31;
...
* Moved Python requirements to ``pip-requirements`` directory and refactored them for backward compatible with Gamestation versions without ``psutil`` yet;
...
* Updated webfont;
* Added link to Gamestation 'virtual gamepad' app on homepage, close #24;
* Update README.rst;
...
* Added ``settings.GAMESTATION_PSUTIL_CPU_INTERVAL`` to define time blocking interval to watch for cpu usage;
...
* Added requirement file for development and Gamestation 3.3.0 beta5;
...
* Added translated link to Gamestation wiki, #14;
...
* Add form config to edit Gamestation configuration #7;
